...
function biddingSteps(t: (k: string) => string): Step[] {
  return [
    ...
    { title: t('supplier.steps.getPaid.title'), desc: t('supplier.steps.getPaid.desc') },
  ]
}
...
const MUTED_STATS = [
  ...
  { key: 'revenue', value: '0', accent: 'secondary' },
] as const
...
function VerifiedDashboard({ data, onBrowseRfqs }: { data: ReturnType<typeof useSupplierDashboard>; onBrowseRfqs: () => void }) {
  const { t } = useTranslation()

  return (
    <>
      <WelcomeHero
        greeting={t('supplier.welcome', { org: data.org.name })}
        ...
        {data.stats.map((stat, index) => {
          ...
          return (
            <div key={stat.key} style={{ animationDelay: `${index * 60}ms` }} className="motion-safe:animate-stepper-in">
              <StatCard icon={<Icon />} value={stat.value} label={t(`supplier.stats.${stat.key}`)} accent={stat.accent} delta={stat.delta} />
            </div>
          )
        })}
      </div>
      ...
      <SectionCard
        title={
          <span className="flex items-center gap-2">
            ...
            <span className="rounded-full bg-status-danger-subtle px-1.5 text-xs font-semibold text-status-danger">{data.actionCount}</span>
          </span>
        }
      >
      ...
      <QuickActions
        items={[
          ...
          { icon: <BarsIcon />, label: t('supplier.actions.submitQuote'), accent: 'success' },
          { icon: <TruckIcon />, label: t('supplier.actions.trackOrders'), accent: 'info' },
          { icon: <AlertTriangleIcon />, label: t('supplier.actions.raiseDispute'), accent: 'danger' },
        ]}
      />
    </>
  )
    ...
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {MUTED_STATS.map((stat) => {
          const Icon = STAT_ICON[stat.key] ?? FileIcon
          return <StatCard key={stat.key} muted accent={stat.accent} icon={<Icon />} value={stat.value} label={t(`supplier.stats.${stat.key}`)} />
        })}
      </div>
